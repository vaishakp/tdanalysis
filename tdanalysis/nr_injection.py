...
class NRInjection:
    ...
    def check_signal_duration(self):
        ...
        if "solarmass" in self.frame:
            ...
            if data_duration != self.analysis_duration:
                raise ValueError(
                    f" The data duration is {data_duration} while the target duration is {self.analysis_duration}"
                )
    ...
    def prepare_signal(self):
        ...
        end_ind = int(
            (self.t_start + self.analysis_duration) * self.target_sampling_frequency
        )
        ...
        if cur != int(self.analysis_duration * self.target_sampling_frequency):
            raise ValueError(
                "The computed length of signal is not consistent with the requested values"
            )
